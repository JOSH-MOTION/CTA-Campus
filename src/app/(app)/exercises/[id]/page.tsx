...
export default function ExerciseSubmissionsPage() {
  ...
  const exercise = exercises.find(a => a.id === exerciseId);
  ...
  const handleGrade = async (submission: Submission) => {
    ...
    try {
      await awardPoint(submission.studentId, 1, 'Class Exercises', activityId);
      ...
      await addNotificationForUser(submission.studentId, {
        title: `Your exercise "${submission.assignmentTitle}" has been graded!`,
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
            ...
            <p className="text-muted-foreground">
                {isLoading ? 'Loading...' : `Reviewing submissions for "${exercise?.title}"`}
            </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A list of all students who have submitted their work for this exercise.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
